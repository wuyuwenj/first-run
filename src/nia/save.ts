...
export async function saveFix(fix: KnownFix): Promise<void> {
  ...
  const tags = ["setup-fix", fix.os, fix.arch, fix.stepName, repoTag].filter(
    (t): t is string => !!t,
  );
...
export async function saveKnowledge(
  ...
  const allTags = [category, ...(tags ?? []), repoTag].filter(
    (t): t is string => !!t,
  );
  const args = [
    "contexts", "save", title,
